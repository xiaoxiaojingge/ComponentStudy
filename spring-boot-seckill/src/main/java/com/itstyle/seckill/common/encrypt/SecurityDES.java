 ...
 *
 * 二：transformation：有两种格式 1：算法/模式/填充方式。如：DES/CBC/PKCS5Padding 2：算法。 如：DES
 *
...
public class SecurityDES {
	 ...
	 * 使用KeyGenerator生成key
	 *
	 * 其中，algorithm支持的算法有：AES、DES、DESEDE、HMACMD5、HMACSHA1、HMACSHA256、RC2等 全部支持的算法见官方文档
	 * https://docs.oracle.com/javase/8/docs/technotes/guides/security/StandardNames.html#
	 * KeyGenerator
	 *
	 */
	...
	/**
	 * 使用SecretKeySpec生成key 一般是从一个文件中读取出key的byte数组，然后根据文件key的算法，构建出key对象
	 */
	...
	/**
	 * 加密，对字符串进行加密，返回结果为byte数组 保存的时候，可以把byte数组进行base64编码成字符串，或者把byte数组转换成16进制的字符串
	 *
