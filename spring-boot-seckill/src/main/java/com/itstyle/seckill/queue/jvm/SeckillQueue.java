 ...
 * 秒杀队列(固定长度为100)
 *
 * @author 科帮网 By https://blog.52itstyle.com 创建时间 2018年5月10日
 */
public class SeckillQueue {
	 ...
	 * 生产入队
	 * @param kill
	 * @throws InterruptedException add(e) 队列未满时，返回true；队列满则抛出IllegalStateException(“Queue
	 * full”)异常——AbstractQueue put(e) 队列未满时，直接插入没有返回值；队列满时会阻塞等待，一直等到队列未满时再插入。 offer(e)
	 * 队列未满时，返回true；队列满时返回false。非阻塞立即返回。 offer(e, time, unit)
	...
	/**
	 * 消费出队 poll() 获取并移除队首元素，在指定的时间内去轮询队列看有没有首元素有则返回，否者超时后返回null take()
