...
/**
 * 高性能，轻量级的分布式内存队列 特性: 1、支持优先级(支持任务插队) 2、延迟(实现定时任务) 3、持久化(定时把内存中的数据刷到binlog日志)
 * 4、预留(把任务设置成预留，消费者无法取出任务，等某个合适时机再拿出来处理) 5、任务超时重发(消费者必须在指定时间内处理任务，如果没有则认为任务失败，重新进入队列)
 * https://beanstalkd.github.io/ https://www.cnblogs.com/xiexj/p/7644999.html
...
public class RedPacketDelayQueue {
	...
	public static void main(String[] args) {
		...
		String msg = "你好, 微信红包";
		/**
		 * priority：优先级 delay：延迟多长时间开始执行，单位秒 ttr: 单位秒，为 consumer 操作设置的 reserve超时时间, 如果
		 * consumer 在这个ttr时间里没有完成 job 并将 job delete掉， 那这个job就会重新被迁回ready状态，再次供消费者执行
		 */
