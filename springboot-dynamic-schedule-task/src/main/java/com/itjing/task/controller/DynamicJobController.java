	...
	public String addDynamicJob() {
		...
		if (insertCount == 0) {
			...
			if (Objects.equals(dynamicJob.getJobStatus(), JobStatusConstant.NORMAL)) {
				SchedulingRunnable task = new SchedulingRunnable(dynamicJob.getBeanName(), dynamicJob.getMethodName(),
						methodParams);
		...
		if (Objects.nonNull(existedDynamicJob)) {
			...
			if (updateCount == 0) {
				...
				if (existedDynamicJob.getJobStatus().equals(JobStatusConstant.NORMAL)) {
					LinkedHashMap existedMethodParams = JSONObject.parseObject(existedDynamicJob.getMethodParams(),
							LinkedHashMap.class);
					SchedulingRunnable task = new SchedulingRunnable(existedDynamicJob.getBeanName(),
							existedDynamicJob.getMethodName(), existedMethodParams);
					cronTaskRegistrar.removeCronTask(task);
				}

				if (Objects.equals(dynamicJob.getJobStatus(), JobStatusConstant.NORMAL)) {
					SchedulingRunnable task = new SchedulingRunnable(dynamicJob.getBeanName(),
							dynamicJob.getMethodName(), methodParams);
					cronTaskRegistrar.addCronTask(task, dynamicJob.getCronExpression());
				}
			}
		}
