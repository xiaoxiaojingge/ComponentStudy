...
import com.itjing.activiti.response.ResponseResult;
...
public class ActivitiController {
	...
	public ResponseResult getProcessList(@PathVariable(name = "key", required = false) String key) {
		...
		if (key != null) {
			definitionList = definitionQuery.processDefinitionKey(key).list();
		}
	...
	public ResponseResult getMyTaskList() {
		...
		// 获取任务名称列表
		List<String> TaskList = tasks.stream().map(Task::getName).collect(Collectors.toList());
	...
	public ResponseResult doTask(@RequestParam(name = "key") String key,
			@RequestParam(name = "assignee") String assigne) {
		List<Task> tasks = taskService.createTaskQuery().processDefinitionKey(key).taskAssignee(assigne).list();
