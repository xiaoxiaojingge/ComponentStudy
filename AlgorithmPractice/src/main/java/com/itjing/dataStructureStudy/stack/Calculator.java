			...
			if (operStack.isOper(ch)) {// 如果是运算符
				if (!operStack.isEmpty()) {// 判断当前字符站是否为空
					...
					if (operStack.priority(ch) <= operStack.priority(operStack.peek())) {
						...
						operStack.push(ch);// 如果当前操作符的优先级大于栈中的操作符,就直接入符号栈
				}
				else
					operStack.push(ch); // 如果为空直接入符号栈
			}
			else {
				// 如果是数,则直接入数栈
				...
				if (index == expression.length() - 1)
					...
					if (operStack.isOper(expression.substring(index + 1, index + 2).charAt(0))) {// 此处是判断是符号入栈,并清空,不是不好就记录下来
																									// 下轮使用
						numStack.push(Integer.parseInt(keepNum));
						keepNum = "";
					}
