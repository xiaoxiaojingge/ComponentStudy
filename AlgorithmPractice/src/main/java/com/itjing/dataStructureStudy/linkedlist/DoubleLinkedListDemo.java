...
class DoubleLinkedList {
	...
	/**
	 * 5. 从双向链表中删除一个节点 说明: 1.对于双向链表,我们可以直接找到要删除的这个节点 2.找到后,自我删除即可
	 */
	public void del(int no) {
		...
		if (flag) {
			...
			if (temp.next != null)
				temp.next.pre = temp.pre;// 将下一个节点的上一个(pre)赋值为当前节点的上一个(pre)
		}
		else
