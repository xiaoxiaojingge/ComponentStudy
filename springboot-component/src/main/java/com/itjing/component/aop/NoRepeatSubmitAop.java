package com.itjing.component.aop;

import com.itjing.component.annotation.NoRepeatSubmit;
import com.itjing.component.common.response.ApiResult;
import com.itjing.component.util.IpUtils;
import com.itjing.component.util.RedisUtil;
...
public class NoRepeatSubmitAop {
     ...
     * com.itjing.component.*.controller： 标识AOP所切服务的包名，即需要进行横切的业务类
     ...
     */
    @Around("execution(* com.itjing.component.controller.*Controller.*(..)) && @annotation(noRepeatSubmit)")
    public Object doAround(ProceedingJoinPoint pjp, NoRepeatSubmit noRepeatSubmit) {
        try {
            ...
            if (redisUtil.hasKey(key)) {
                ...
                if ((now - lastTime) > noRepeatSubmit.interval()) {
                    // 非重复提交操作 - 重新记录操作时间
                    ...
                    return ApiResult.fail(noRepeatSubmit.message());
                }
            } else {
                // 这里是第一次操作
                redisUtil.set(key, String.valueOf(now));
