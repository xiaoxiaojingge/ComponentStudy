 ...
 * <p>
 * 为了解决rabbit繁琐的配置交换机，队列，及队列绑定。 实现了公共方法统一消息队列发送消息方法
 * <p>
 * 使用步骤： 1 在application主方法类上开启注解@EnableMq 2 配置文件配置rabbitmq host ，port virtual-host 等配置 3
 * pom引入spring-boot-starter-amqp依赖 4 自定义 交换机{@link ExchangeEnum}，普通队列{@link QueueEnum}
 * ，延迟队列{@link DelayQueueEnum} 5 发送消息 mqService.send(msg,queueEnum) 6
 * 自定义监听消息。例如@RabbitListener(queues = "payment_service_notify_dead_queue")
 */
public interface MqService {
	...
	/**
	 * 延迟发送 消息发送至延时队列中，不要消费，过期后消息进入与之绑定的死信队列中 消息消费需监听死信队列
	 * @param msg
