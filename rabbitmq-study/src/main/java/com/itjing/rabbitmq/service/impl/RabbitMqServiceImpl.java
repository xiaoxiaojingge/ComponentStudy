...
public class RabbitMqServiceImpl implements MqService {
	...
	private void createMyExchange(ExchangeEnum exchangeEnum) throws Exception {
		switch (exchangeEnum.getType()) {
			case direct:
				rabbitAdmin
					.declareExchange(new DirectExchange(exchangeEnum.getName(), exchangeEnum.getDurable(), false));
				break;
			case topic:
				rabbitAdmin
					.declareExchange(new TopicExchange(exchangeEnum.getName(), exchangeEnum.getDurable(), false));
				break;
			case fanout:
				rabbitAdmin
					.declareExchange(new FanoutExchange(exchangeEnum.getName(), exchangeEnum.getDurable(), false));
				break;
			default:
	...
	private void createMyQueue(QueueEnum queueEnum) throws Exception {
		if (null != queueEnum.getArguments() && queueEnum.getArguments().size() > 0) {
			rabbitAdmin.declareQueue(
					new Queue(queueEnum.getName(), queueEnum.getDurable(), false, false, queueEnum.getArguments()));
			return;
		}
	...
	private void createMyDelayQueue(DelayQueueEnum delayQueueEnum) throws Exception {
		Map<String, Object> params = DelayQueueEnum.params(delayQueueEnum.getDeadExchangeEnum().getName(),
				delayQueueEnum.getDeadQueueEnum().getName(), delayQueueEnum.getTtl());
	...
	private void bindExchangeAndQueue(ExchangeEnum exchangeEnum, QueueEnum queueEnum) {
		...
		rabbitAdmin.declareBinding(new Binding(queueEnum.getName(), Binding.DestinationType.QUEUE,
				exchangeEnum.getName(), queueEnum.getName(), null));
	}
	...
	private void bindDelayExchangeAndQueue(ExchangeEnum exchangeEnum, DelayQueueEnum delayQueueEnum) {
		...
		rabbitAdmin.declareBinding(new Binding(delayQueueEnum.getName(), Binding.DestinationType.QUEUE,
				exchangeEnum.getName(), delayQueueEnum.getName(), null));
	}

	/**
	 * 发送消息LocalDateTime时间格式化问题 这里ObjectMapper需要自定义序列化配置
	 */
