...
public class XmlUtil {
	...
	/**
	 * 创建一个配置好的XmlMapper实例。 该方法配置了XmlMapper的各种设置，包括序列化和反序列化的行为。
	...
	private static JavaTimeModule configTimeModule() {
		...
		javaTimeModule.addSerializer(Date.class, new DateSerializer(false, new SimpleDateFormat(dateFormat)));
	 ...
	 * 将给定的对象转换为XML格式的字符串。
	 * <p>
	 * 使用Jackson库的XMLMapper将Java对象序列化为XML格式的字符串。 此方法封装了序列化过程，并在发生异常时抛出运行时异常。
	 ...
	 * <p>
	 * 本方法使用Jackson库的XMLMapper来解析XML字符串，将其映射为Java对象。 这种方式提供了便利的XML到对象的转换，适用于处理XML格式的数据。
