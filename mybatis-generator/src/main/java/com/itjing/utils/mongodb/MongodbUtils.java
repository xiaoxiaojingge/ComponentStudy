...
public class MongodbUtils {
	...
	/**
	 * 保存对象List到指定集合中 也可以在实体类上使用@Document(collection=“集合名称”)指定集合名称，未指定则默认实体类的类名为集合名称
	 * @param entiys 实体集合
	 */
	 ...
	 * @param
	 */
	public static <T> List<T> findSortById(Class<T> entityClass, String collName, Sort.Direction direction,
			String... sortFields) {
	 ...
	 * @param collName 集合名称
	 * @param map Map<查询条件key,查询条件value> 返回字段的时候id默认为返回，不返回id则field设置
	 * fieldObj.put("id",false)
	 * @return
	 */
	public static <T> List<T> findDesignField(List<String> fields, Map<String, Object> map, Class<T> entityClass,
			String collName, boolean returnId) {
	 ...
	 * @return
	 */
	public static <T> List<T> findLikeByParam(String key, String param, String collName, String sortField,
			Sort.Direction direction, Class<T> entityClass) {
	 ...
	 * @return
	 */
	public static <T> List<T> findSortByParam(Class<T> entityClass, String collName, Map<String, Object> map,
	...
	/**
	 * 范围查询 查询大于等于begin 小于等于end范围内条件匹配得数据并排序
	 * @param entityClass 数据对象
	 * @param collName 集合名称
	 * @param map Map<"查询条件key"，查询条件值> map
	 * @param sortField 排序字段
	 * @param direction 排序方式 Direction.asc / Direction.desc
	 * @param rangeCriteria 示例： lt小于 lte 小于等于 gt大于 gte大于等于 eq等于 ne不等于 Criteria
	 * rangeCriteria=Criteria.where("createDate").gte(begin).lte(end));
	 ...
	 * @param collName 集合名称
	 * @param map 查询条件map
	 * @param rangeCriteria 范围条件 Criteria rangeCriteria=
	 * Criteria.where("数据库字段").gt/gte（起始范围）.lt/lte（结束范围）
	 * @return
	 */
	public static Long countRangeCondition(Class<?> clazz, String collName, Criteria rangeCriteria,
	 ...
	 * @return
	 */
	public static <T> T findSortFirst(Class<T> clazz, Map<String, Object> map, String collName, String field,
	 ...
	 */
	public static <O> List<O> findSum(Class<O> clazz, Map<String, Object> map, String collName, String sumField,
		...
		return mongodbUtils.mongoTemplate.aggregate(Aggregation.newAggregation(match, count), collName, clazz)
			.getMappedResults();
	}
