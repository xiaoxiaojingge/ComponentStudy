...
public class ExcelOfCsvUtil {
	...
	public static void main(String[] args) throws FileNotFoundException {
		// csv文件地址
		/*
		 * List<String> strs = readCsv(null); strs.stream().forEach(str -> { // 一般是逗号分隔的
		 * String[] field = str.split(","); Stream.of(field).forEach(data -> {
	 ...
	 */
	public static <T> void generateCsvFile(List<T> exportResults, String fileName, String[] header)
			throws IOException, CsvDataTypeMismatchException, CsvRequiredFieldEmptyException {
		Writer writer = new FileWriter(fileName);

		// 设置显示的顺序
		/*
		 * String[] columnMapping = {"name", "age"}; ColumnPositionMappingStrategy<CsvDTO>
		 * mapper = new ColumnPositionMappingStrategy<>(); mapper.setType(CsvDTO.class);
