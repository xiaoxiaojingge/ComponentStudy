...
public class LocalDateTimeUtil {
    /**
     * 1.	    adjustInto	把指定的值赋值给传入的对象，使其拥有相同的时间。
     * 2.	    atOffset	将LocalDateTime与偏移量(offset)组合以创建OffsetDateTime。
     * 3.	    atZone	结合LocalDateTime和指定时区创建一个ZonedDateTime
     * 4.	    compareTo	比较两个LocalDateTime
     * 5.	    format	格式化LocalDateTime生成一个字符串
     * 6.	    from	转换TemporalAccessor为LocalDateTime
     ...
     * 19.	    isAfter	判断LocalDateTime是否在指定LocalDateTime
     * 20.	    isBefore	判断LocalDateTime是否在指定LocalDateTime
     ...
     * 33.	    of	根据年、月、日、时、分、秒、纳秒等创建LocalDateTime
     * 34.	    ofEpochSecond	根据秒数(从1970-01-0100:00:00开始)创建LocalDateTime
     * 35.	    ofInstant	根据Instant和ZoneId创建LocalDateTime
