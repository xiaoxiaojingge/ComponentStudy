...
public class DownloadUtil {
     ...
     * @param delFlag    是否删除文件
     */
    public  void download(File file, String returnName, HttpServletResponse response, boolean delFlag) {
    ...
    public void prototypeDownload(File file, String returnName, HttpServletResponse response, boolean delFlag) {
        ...
        try {
            ...
            while ((readLength = inputStream.read(buf, 0, length)) != -1) {
            ...
            try {
                ...
                log.error("下载文件IO读写失败" + e.getMessage());
            }
            try {
                outputStream.close();
            } catch (IOException e) {
    ...
    /**
     * @param baos 将文件内容写入ByteArrayOutputStream
     * @param response  HttpServletResponse	写入response
     * @param returnName   返回的文件名
     */
    public void download(ByteArrayOutputStream baos, HttpServletRequest request, HttpServletResponse response, String returnName) throws IOException {
        response.setContentType("application/octet-stream;charset=utf-8");
