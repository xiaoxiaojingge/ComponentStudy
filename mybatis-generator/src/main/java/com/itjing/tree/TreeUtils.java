...
public class TreeUtils {
	...
	public static List<MenuVo> getMenusVo() {
		List<MenuVo> menus = Lists.newArrayList(new MenuVo(1, "A公司", 0), new MenuVo(2, "a销售部", 14),
				new MenuVo(3, "财税部", 1), new MenuVo(4, "商务部", 1), new MenuVo(5, "综合部", 1), new MenuVo(6, "a销售1部", 2),
				new MenuVo(7, "a销售2部", 2), new MenuVo(8, "a销售3部", 2), new MenuVo(9, "a销售4部", 2),
				new MenuVo(10, "b销售部", 14), new MenuVo(11, "b销售1部", 10), new MenuVo(12, "b销售2部", 10),
				new MenuVo(13, "人事部", 1), new MenuVo(14, "销售部", 1));
		return menus;
	}

	public static List<MenuNode> getMenuNode() {
		List<MenuNode> menus = Lists.newArrayList(new MenuNode(1, "A公司", 0), new MenuNode(2, "a销售部", 14),
				new MenuNode(3, "财税部", 1), new MenuNode(4, "商务部", 1), new MenuNode(5, "综合部", 1),
				new MenuNode(6, "a销售1部", 2), new MenuNode(7, "a销售2部", 2), new MenuNode(8, "a销售3部", 2),
				new MenuNode(9, "a销售4部", 2), new MenuNode(10, "b销售部", 14), new MenuNode(11, "b销售1部", 10),
				new MenuNode(12, "b销售2部", 10), new MenuNode(13, "人事部", 1), new MenuNode(14, "销售部", 1));
		return menus;
	}
	...
	/**
	 * 根据所有树节点列表，按默认条件生成含有所有树形结构的列表 主要用于组建简单树形结构
	 ...
	 */
	@SuppressWarnings("all")
	public static <T extends TreeNode, E extends TreeNode<?, ?, LC>, LC> void getChildren(T parent, List<E> allData,
			LC leafConfition) {
		List<E> children = new ArrayList<>();
	 ...
	 * @return
	 */
	public static <T extends TreeNode<?, ?, LC>, LC> List getTreeByNode(T root, List<T> allData, boolean isParent,
			LC leafCondition) {
		List<T> tree = new ArrayList<>();
		if (isParent) {
			root.setLevel(0);
	 ...
	 * @return
	 */
	public static <T, E extends TreeNode<?, ?, LC>, LC> List getTreeByNodeId(T rootId, List<E> allData,
			boolean isParent, LC leafCondition) {
		List<E> tree = new ArrayList<>();
		E root = null;
		for (Iterator<E> iterator = allData.iterator(); iterator.hasNext();) {
			E node = iterator.next();
	 ...
	 * @return 返回一个结果集的map
	 */
	public static <E extends TreeNode<?, ?, ?>, K, V> Map<K, V> traverseTree(List<E> tree,
			FunctionHandle<E, K, V> handle) {
	 ...
	 * @param <E> 集合类型
	 */
	public static <E extends TreeNode<?, ?, ?>> void recursiveTree(List<E> children, Map resultMap,
