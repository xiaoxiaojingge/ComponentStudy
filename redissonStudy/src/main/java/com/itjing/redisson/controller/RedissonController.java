...
public class RedissonController {
	...
	/**
	 * 最佳实战： 使用 lock.lock(xxx, TimeUnit.SECONDS); 因为省掉了整个续期操作，我们只要保证自动解锁时间大于业务执行时间即可。
	 * @return
	 */
	@GetMapping("/hello")
	public String hello() {
	...
	/*---------------------读写锁测试-----------------------*/
	/**
	 * 写锁，保证一定能读到最新数据，修改期间，写锁是一个排它锁（互斥锁、独享锁）。读锁是一个共享锁。 写锁没有释放，读就必须等待。 读 + 读：相当于无锁，并发读，只会在
	 * redis 中记录好所有当前的读锁。他们都会同时加锁成功。 写 + 读：等待写锁释放 写 + 写：阻塞方式 读 + 写：有读锁，写也需要等待。
	 * 只要有写的存在，都必须等待
	 * @return
	 */
	@GetMapping("/write")
