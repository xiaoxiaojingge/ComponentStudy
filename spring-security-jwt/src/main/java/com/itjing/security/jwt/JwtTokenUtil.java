...
public class JwtTokenUtil {
	...
	/**
	 * 为JWT基础信息加密和解密的密钥 在实际生产中通常不直接写在配置文件里面。而是通过应用的启动参数传递，并且需要定期修改。
	 */
	...
	/**
	 * SecretKey 根据 SECRET 的编码方式解码后得到： Base64 编码：SecretKey key =
	 * Keys.hmacShaKeyFor(Decoders.BASE64.decode(secretString)); Base64URL 编码：SecretKey
	 * key = Keys.hmacShaKeyFor(Decoders.BASE64URL.decode(secretString)); 未编码：SecretKey
	...
	private static SecretKey getSecretKey(String secret) {
		...
		/*
		 * byte[] encodeKey = Decoders.BASE64.decode(secret); return
		 * Keys.hmacShaKeyFor(encodeKey);
		 */
	}
