...
/**
 * @Description: 创建pdf并且插入表格
 * @Author: lijing
 * @CreateTime: 2022-12-02 11:27
 */
public class CreatePdfAndTable {
	...
	public void createPdfFile(HttpServletResponse response) throws IOException, DocumentException {
		...
		s.setField("type", "日报");
		s.setField("createTime", DateUtils.currentTime());
	...
	public void createTableYq(PdfWriter writer, Document document) throws DocumentException, IOException {
		...
		java.util.List<YqTable> yqTables = Lists.newArrayList();
		for (int i = 1; i <= 1000; i++) {
	 ...
	 * 创建表格跟表格之间的空白间隔
	 */
	public void createBlankTable(PdfWriter writer, Document document, BaseFont font, int height)
