...
public class fastjson漏洞分析 {
	...
	public void fastjson_test5() {
		...
		System.out.println("str类型json数据转换成class类型对象：" + JSON.parseObject(value, Student.class)); // str类型json数据转换成class类型对象：Student(id=1,
																									// name=jack,
																									// age=24)
	}

	/**
	 * 通过上面代码,我们可以发现一个重点: fastjson会处理字符串类型的json数据,上面的value变量是字符串类型,这对我们后续漏洞分析很有帮助
	 */
	...
	public void fastjson_test7() {
		...
		/**
		 * 我们像前面这样写,会发现最后字符串json没有转换成对象 因为fastjson找不到我们要转换的json数据在哪个类,这里我们要声明类型
		 */
