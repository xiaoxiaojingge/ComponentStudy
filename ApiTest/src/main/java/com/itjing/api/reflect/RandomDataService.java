...
public class RandomDataService {
	...
	private Object populateDataOrigin(String columnName, Class<?> propertyType) {
		...
		if (propertyType == String.class) {
			...
			else if (lowerCase.contains("idcard")) {
			...
			else if (lowerCase.contains("name") || lowerCase.contains("user")) {
				value = RandomUtil.username();
			}
			else if (lowerCase.contains("address")) {
				value = RandomUtil.address();
			}
			else if (lowerCase.contains("uuid")) {
				value = UUID.randomUUID().toString().replace("-", "");
			}
			else if (lowerCase.contains("job")) {
				value = RandomUtil.job();
			}
			else if (lowerCase.contains("status") || lowerCase.contains("state")) {
				value = RandomUtil.status("1", "2", "3");
			}
			else if (lowerCase.contains("time") || (lowerCase.contains("date") && !lowerCase.equals("update"))
					|| lowerCase.contains("birthday")) {
				// 这里可以做时间格式转换,获取字段上的配置
				value = DateFormatUtils.ISO_DATETIME_FORMAT.format(RandomUtil.date());
			}
			else if (lowerCase.contains("lat")) {
				String randomLongLat = RandomUtil.randomLongLat(25, 115, 26, 160);
				value = StringUtils.split(randomLongLat, ",")[1];
			}
			else if (lowerCase.contains("lon") || lowerCase.contains("lang")) {
				String randomLongLat = RandomUtil.randomLongLat(25, 115, 26, 160);
	...
	/**
	 * 判断是否是原始型扩展 包含 原始型及包装类,String,Date,BigDecimal,Array 数组也当原始型处理,因为数组类型必须要匹配,不能自动拆箱装箱
	 * @param propertyType
	 * @return
	 */
	private boolean isPrimitiveExtend(Class<?> propertyType) {
		return ClassUtils.isPrimitiveOrWrapper(propertyType) || propertyType == String.class
				|| propertyType == Date.class || propertyType == BigDecimal.class || propertyType.isArray();
	}
