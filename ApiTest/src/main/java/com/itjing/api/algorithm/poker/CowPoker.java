package com.itjing.api.algorithm.poker;

import com.itjing.api.sometest.RegexValidate;
 ...
 * 功能:用于斗牛的一组扑克,五张牌<br/>
 * 规则选用百度百科中的规则
 * url:http://baike.baidu.com/link?url=qdXWYwyrd8XbQYa6NQO6B66lKoNKpsB3Lvw0vY8dnp4V9kMj3m7qOuY8pa4SPYlwttTH_I1jBFxkQqrJQ8ya0a
 * <br/>
...
public class CowPoker {
	...
	public Map<String, String> level() throws Exception {
		if (pokers.size() != 5) {
			throw new Exception("斗牛牌数必须是 5 现在是:" + pokers.size());
		}
		...
		if (wrapSymbols.matches(
				"(\\{[1-9aAjqkJQK]{1}0?\\})(\\{[1-9aAjqkJQK]{1}0?\\})(\\2\\2\\2)|(\\{[1-9aAjqkJQK]{1}0?\\})(\\4\\4\\4)(\\{[1-9aAjqkJQK]{1}0?\\})")) {
			combination.put(pokerSymbols, "14");
			return combination;
		}
		int gt10Count = 0;
		int fiveSmallCowCount = 0;
		boolean isFiveSmallCowPre = true;
			...
			for (int j = i + 1; j < pokerValuesArray.length; j++) {
				for (int k = j + 1; k < pokerValuesArray.length; k++) {
					...
					if (sum % 10 == 0) {
						...
						String key = String.valueOf(pokers.get(i) + "," + pokers.get(j) + "," + pokers.get(k) + "#"
								+ pokers.get(othersPos[0]) + "," + pokers.get(othersPos[1]));
	...
	public String pokerSymbols() {
		if (!RegexValidate.isEmpty(pokers)) {
