...
/**
 * 算法动态展示 https://www.cnblogs.com/onepixel/articles/7674659.html
 *
 * 冒泡排序 插入排序 希尔排序 选择排序 快速排序 归并排序 堆排序 计数排序/桶排序 基数排序
 */
public class SortAll {
	...
	/**
	 * 从第 2 个元素开始, 找到元素插入位置 前面的元素是已经排好序的,往前找位置的时候,需要腾出位置给新元素
	 */
	@Test
	...
	/**
	 * 使用哨兵的插入排序 每次循环少了一次判断 将 k >= 1 && nums[k - 1] > currValue 减少为一次判断
	 */
	@Test
	...
	/**
	 * 选择排序 将序列分为有序集和无序集 , 每次从无序集中找出最小的放在有序集中 可以将有序集和无序集看做一个集合,用交换法将小数排到前面去
	...
	/**
	 * 快速排序其实就是找数据的基准位置, 这里用的挖坑法 选定第一个数为基准数,这里相当于挖了一个坑 , 从右边往前找,如果数比基准大则往前移 ,如果比基准小则填入坑中;
	 * 这个数的位置成为新的坑 然后从左边往后面找,如果数比基准小,则往后移动,如果比基准大则填入坑中,它的位置成为新的坑 , 直到左右指针一致,则找到基准位置
	 * 然后对左右两边的数据递归找基准位置,则排好序了 需要定义的数据有 , 坑的位置 index , 左索引 left , 右索引 right , 基准值 base
	 *
	 * 我在写的时候写出了几个 bug 1. 在递归的时候,需要判断 right 是不是大于 left ,不要在 right == left 的时候还去递归去排序; 即
	 * if(right > left ) 2. 不一定在往前移的时候总会有一个不正确的值,有可能是正常的顺序,所在在 while (right > left &&
	 * array[right] > base) 结束后还需要判断当前值和 base 值的比较 3. 最后在递归的时候, left 和 right
	 * 需要用之前的值,需要保存起来 ,不要用修改后的值来操作
	 * @param array
	 * @return
	 */
	...
	/**
	 * 希尔排序 将原数据分成多个组 , 然后对组内数据进行插入排序 然后缩小间隙 , 再次对组内数据进行插入排序 , 这里组数会逐渐减少 最后只有一个组的时候,进行插入排序
	 *
	 * 没看懂 TODO
	 */
	@Test
	...
	/**
	 * 归并排序关键过程在归并的过程 , 需要借助一个临时数组 , 然后有 3 个指针, 分别指向左索引,右索引,和辅助数据的数据位置
	...
	/**
	 * 堆排序 大顶堆: 二叉树结构的顶点都要比子节点大的叫大顶堆 小顶堆: 二叉树结构的顶点都要比子节点小的叫小顶堆
	 *
	 * 二叉树可以用数组表示 , 我们用层序遍历出来
	 *
	 * 1. 构建大顶堆 2. 摘出最大元素与数组最后一个元素交换 3. 重复上述过程
	 *
	 * 有点割韭菜的感觉
	 */
	@Test
	...
	/**
	 * 计数排序,这个算法是用空间来换取时间, 需要一个比较大的数组来存储数据,将数据的值放在大数组的下标位置; 然后如果值相等则计数加一,否则计数为 0
	 * 然后再把数据从数组中反向填充到原数组
	 *
	 * 优化 : (这个优化就是桶排序 , 有点像 hashmap 了) 第二步中我们可以使用函数将数据固定在一定空间范围内 ,
	 * 然后在取出的时候再使用别的排序对这一小范围内的数据进行排序写出
	 */
	@Test
	...
	/**
	 * 基数排序 可以先排序个位数 , 然后排序十位数 , 然后排序百位 并且每次只排序一位 , 需要的空间最多是 10 个数组空间 , 可以使用计数排序
	 */
	@Test
	...
	/**
	 * 对于基数排序中,需要保存原数据和重复的数的数量 所以这里用对象来记录
	 */
	@Getter
	...
	/**
	 * 返回数据 num 的 k 位是多少; k 从 0 开始 数组反转 , 从最低位取起, 而不是最高位
	 * @param num
	 * @param k
	 * @return
	 */
