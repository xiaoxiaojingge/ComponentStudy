...
public class QQCowGame {
	 ...
	 * @throws Exception
	 */
	private void poker4Probability(Poker[] pokers, CopyOnWriteArrayList<Poker> otherHavePokers,
			List<Poker> copyAllPoker, long[] levelSum) throws Exception {
	...
	//
	// Map<String, String> probability =
	// qqCowGame.averageProbability(poker1,poker2,poker3,poker4);
	...
	public static void main(String[] args) throws NumberFormatException, Exception {
		...
		while (choice) {
			...
			try {
				...
				// Poker poker = new
				// Poker(Integer.parseInt(split2[0]),Integer.parseInt(split2[1]));
				// pokers.add(poker);
				// }
				// poker1 = pokers.get(0);
				// poker2 = pokers.get(1);
				...
				if (split.length == 6) {
					probability = qqCowGame.averageProbability(poker1, poker2, poker3, poker4,
							new Poker(split[5], Poker.HEARTS));
					Iterator<Entry<String, String>> it = probability.entrySet().iterator();
					for (; it.hasNext();) {
						Entry<String, String> next = it.next();
						System.out
							.println(next.getKey() + "--" + CowPoker.LEVELS.get(Integer.parseInt(next.getValue())));
					}
					continue;
				}
				probability = qqCowGame.averageProbability(poker1, poker2, poker3, poker4);
