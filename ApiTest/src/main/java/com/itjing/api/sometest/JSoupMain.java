...
public class JSoupMain {
	...
	public static void main(String[] args) {
		try {
			JSoupMain jSoupDemo = new JSoupMain();
	...
	private void handlerTag(String tag, int pn) throws IOException {
		...
		Document document = Jsoup.connect(requestUrl).userAgent("Mozilla").timeout(3000).get();
		// 获取当前页问题列表
	...
	private void readQuestion(String href) throws IOException {
		Document document = Jsoup.connect(href).userAgent("Mozilla").timeout(3000).get();
		parseAsk(document);
