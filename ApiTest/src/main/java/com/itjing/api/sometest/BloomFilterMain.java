...
/**
 * 布隆过滤器测试 https://www.jianshu.com/p/2104d11ee0a2 布隆过滤器的特点就是能判断元素一定不存在,但元素是否存在是有误判的 它使用 k
 * 个函数计算 元素 在 bit 数组上的位置,并把其置于 1 ,下次判断元素是否存在时,只要再次计算值,只要其中一个位置为 0 ,则元素一定不存在,反之可能存在 我们需要预估
 * bit 数组的大小和 函数的个数 ,可以使用公式 k = (m/n)ln2 | k 为函数个数 , m 为 bit 数组长度 , n 为插入元素个数
 */
