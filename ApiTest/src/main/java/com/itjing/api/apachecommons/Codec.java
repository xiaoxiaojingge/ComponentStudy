...
public class Codec {
	 ...
	 *
	 * bas64 是指使用 6 个 bit 为一个单元,对应某个可打印字符,因为 2^6 = 64 所以叫 base64 源数据最小单位是 byte ,所以 8 bit 和
	 * 6 bit 最大公约数为 24 , 即刚好可以 3 个 byte 对应四个可打印字符,转换后的大小为源大小的 4/3 又因为源字节数不一定刚好是 3
	 * 的倍数,所以当不够时在末尾补 0 ,base64 中用 = 填充
	 */
	@Test
	 ...
	 * 功能 : 用于把字节转换成可打印字符 , 转换后会增大数据体积
	 *
	 * hex 在网上没有找到,但照这个情况应该是 4bit 一编码成一个字母,"中文" utf-8 编码是 6 字节,所以最终生成 12 位 hex 编码原理 取 1 字节
	 * 的前4 位和后 4 位在 DIGITS 表中的映射,可自定义 digits 表,一般用数字和字母的组合
	 *
	...
	/**
	 * md5 本身是没有密匙的,可以自定义一个密匙和密码做一定规则的合并,然后调用 md5 可以达到自定义密匙的作用 md5 生成的是 byte [] , 可以使用
	 * base64 或 hex 工具转成可见字符
	 *
